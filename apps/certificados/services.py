"""
Service de Certificados de Instalación (Django ORM + SQLite y Storage).
"""
...
class CertificadoService:
    ...
    def guardar_certificado(
        ...
        ruta_certificado = BASE_STORAGE / str(año) / location
        ruta_certificado.mkdir(parents=True, exist_ok=True)
        ...
        archivo_json = ruta_certificado / "certificado.json"
        with open(archivo_json, "w", encoding="utf-8") as archivo:
            json.dump(contenido, archivo, indent=4, ensure_ascii=False)

        # Guardar / Actualizar en SQLite
        try:
            datos_gen = contenido.get("datos_generales", {})
            CertificadoInstalacion.objects.update_or_create(
                location=location,
                año=año,
                defaults={
                    "nombre_centro": datos_gen.get("nombre_centro", ""),
                    "empresa": datos_gen.get("empresa", ""),
                    "fecha_instalacion": datos_gen.get("fecha_instalacion", ""),
                    "tecnico_visita": datos_gen.get("tecnico_visita", ""),
                    "numero_ficha": datos_gen.get("numero_ficha", ""),
                    "data": contenido,
                }
            )
        ...
        if len(carpetas) > max_guardados:
            for _, carpeta in carpetas[max_guardados:]:
                try:
                    ...
                    # También limpiar en SQLite si aplica
                    try:
                        CertificadoInstalacion.objects.filter(location=carpeta.name, año=año).delete()
                    except Exception:
                        pass
                except Exception as exc:
    ...
    def listar_certificados(
        ...
        try:
            certs_db = list(CertificadoInstalacion.objects.filter(año=año).values_list("location", flat=True))
            if certs_db:
                return sorted(certs_db)
        except Exception:
            pass
    ...
    def cargar_certificado(
        ...
        try:
            cert_obj = CertificadoInstalacion.objects.filter(location=location, año=año).first()
            if cert_obj and cert_obj.data:
                return cert_obj.data
        except Exception:
            pass

        archivo_json = BASE_STORAGE / str(año) / location / "certificado.json"
        if not archivo_json.exists():
            raise FileNotFoundError(f"No existe certificado para {location}")
    ...
    def eliminar_certificado(
        ...
        """Elimina la carpeta completa del certificado (JSON, PDF y evidencias) y su registro en SQLite."""
        eliminado = False
        try:
            CertificadoInstalacion.objects.filter(location=location, año=año).delete()
            eliminado = True
        except Exception:
            pass
